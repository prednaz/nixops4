...
impl EvalState {
    ...
    pub fn new_value_int(&self, i: Int) -> Result<Value> {
        let v = unsafe {
            let value = self.new_value_uninitialized();
            raw::nix_init_int(self.context.ptr(), value.raw_ptr(), i);
            value
        };
        self.context.check_err()?;
        Ok(v)
    ...
    pub fn realise_string(
        ...
        let s = unsafe {
            let start = nix_realised_string_get_buffer_start(rs) as *const u8;
        ...
        let paths = unsafe {
            let n = nix_realised_string_get_store_path_count(rs);
...
mod tests {
    ...
    fn eval_state_new_and_drop() {
        gc_registering_current_thread(|| {
            ...
            let _e = EvalState::new(iter::empty(), store).unwrap();
        })
        .unwrap();
    }
    ...
    fn eval_state_lookup_path() {
        ...
        gc_registering_current_thread(|| {
            let es = EvalState::new(iter::empty(), Store::open("auto").unwrap()).unwrap();
    ...
    fn eval_state_eval_from_string() {
        gc_registering_current_thread(|| {
            let store = Store::open("auto").unwrap();
            let es = EvalState::new(iter::empty(), store).unwrap();
